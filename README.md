...
along the path it visits than is possible with `traceroute`, like many of the 
`SNMP MIB` variables (e.g., CPU load in the last 1, 5, and 15 minutes, etc). 
Finally, in open networks between different companies, `traceroute` doesn't 
need to traverse, in general, a pre-established set of hops A, B, C, ..., Z, 
unless the `IP loose-source-routing` option is set in the request (and honored
by intermediate routers between those companies), which 
